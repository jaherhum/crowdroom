...
class CreateSession(BaseModel):
    ...
    current_platform: StreamingPlatforms = Field(..., description="The streaming platform currently in use.")
    playback_status: PlaybackStatus = Field(..., description="The initial playback status of the session.")
...
class ReadSession(BaseModel):
    ...
    Attributes:
        ...
        current_song_id (Optional[UUID]): The ID of the song currently playing, if any.
    ...
    room_id: UUID = Field(..., description="The identifier of the room this session belongs to.")
    current_platform: StreamingPlatforms = Field(..., description="The streaming platform currently in use.")
    current_song_id: Optional[UUID] = Field(None, description="The ID of the song currently being played.")
    playback_status: PlaybackStatus = Field(..., description="The current playback status.")
    last_updated: datetime = Field(..., description="The timestamp of the last recorded activity in this session.")
...
class UpdateSession(BaseModel):
    ...
    Attributes:
        current_song_id (Optional[UUID]): The new song ID to be played.
        playback_status (Optional[PlaybackStatus]): The new playback status (e.g., play, pause, stop).
    """
    current_song_id: Optional[UUID] = Field(None, description="The ID of the new song to play.")
    playback_status: Optional[PlaybackStatus] = Field(None, description="The updated playback status.")