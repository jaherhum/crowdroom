...
def _settings(frontend_url: str = "http://localhost:8000") -> Settings:
    return Settings(
        SECRET_KEY="x",
        ENCRYPTION_KEY="x" * 32,
...
class TestCreateQRInvite:
    ...
    def test_create_qr_invite_uses_long_token_and_24h_ttl(
        self, mock_invite_service, host_user_id, room_id
    ):
        mock_invite_service.create_invite.return_value = self._stub_invite()
        service = InviteQRService(mock_invite_service, _settings())
    ...
    def test_create_qr_invite_strips_trailing_slash_from_frontend_url(
        ...
        service = InviteQRService(
            mock_invite_service, _settings("http://example.test/")
        )
...
class TestSendToDevice:
    ...
    def _service_with_client(self, mock_invite_service, fake_client):
    ...
    def test_appends_path_when_root(self, mock_invite_service, host_user_id, room_id):
        mock_invite_service.create_invite.return_value = self._stub_invite()
        fake = _FakeAsyncClient(response=_ok_response())
        service = self._service_with_client(mock_invite_service, fake)
        payload = SendToDeviceRequest(device_url="http://192.168.1.42")

        anyio.run(service.send_to_device, room_id, host_user_id, payload)

        sent_url, sent_body = fake.requests[0]
        assert sent_url == f"http://192.168.1.42{DEVICE_QR_PATH}"
        assert sent_body["token"] == "x" * 32
        assert sent_body["url"] == f"http://localhost:8000/invite?token={'x' * 32}"
    ...
    def test_keeps_custom_path(self, mock_invite_service, host_user_id, room_id):
        mock_invite_service.create_invite.return_value = self._stub_invite()
        fake = _FakeAsyncClient(response=_ok_response())
        service = self._service_with_client(mock_invite_service, fake)
    ...
    def test_accepts_lan_addresses(
        self, mock_invite_service, host_user_id, room_id, url
    ):
        mock_invite_service.create_invite.return_value = self._stub_invite()
        fake = _FakeAsyncClient(response=_ok_response())
        service = self._service_with_client(mock_invite_service, fake)
    ...
    def test_rejects_public_ip(self, mock_invite_service, host_user_id, room_id):
