...
async function localLogin() {
  error.value = '';
  try {
    const result = await apiPost('/auth/local-login', { username: localUsername.value.trim() });
...
async function onlineRegister() {
  error.value = '';
  try {
    await apiPost('/auth/register', { username: regUsername.value.trim(), email: regEmail.value.trim(), password: regPassword.value });
    activeTab.value = 'login';
    identifier.value = regUsername.value.trim();
  } catch (err) {
